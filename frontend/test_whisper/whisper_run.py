...
def parse_args():
    parser = argparse.ArgumentParser(
        description=("Whisper cached decoder-step runner using tvm.relax.frontend.nn and tokffi detokenization.")
    )
    ...
    parser.add_argument(
        "--print-ids",
        action="store_true",
        help="Print generated token IDs in addition to the decoded text.",
    )
...
def build_tokffi_tokenizer(processor, lib_path: Path, tokenizer_dir: Path | None):
    ...
    tokenizer = TokFFITokenizer(tokenizer_obj, decode_fn)
...
def main():
    ...
    tokffi_tokenizer, tokffi_tmpdir = build_tokffi_tokenizer(
        processor,
        args.tokffi_lib,
        args.tokffi_tokenizer_dir,
    )
    ...
    try:
        ...
        preprocess_vm, _ = compile_nn_module_to_vm(preprocess_model, target, dev)
        encoder_vm, encoder_params_tvm = compile_nn_module_to_vm(encoder_model, target, dev)
        cross_kv_vm, cross_kv_params_tvm = compile_nn_module_to_vm(cross_kv_model, target, dev)
        decoder_vm, decoder_params_tvm = compile_nn_module_to_vm(decoder_model, target, dev)

        preprocess_out = preprocess_vm["forward"](
            to_tvm_tensor(waveform_fixed, dev),
            to_tvm_tensor(valid_samples, dev),
        )
        input_features_tvm, _ = unwrap_vm_outputs(preprocess_out)
        ...
        cross_cache_out = cross_kv_vm["forward"](
        ...
        for gen_idx in range(int(args.max_new_tokens)):
            ...
            generated_content_ids.append(next_id)
        ...
        full_ids = prompt_ids + generated_content_ids
        content_ids = strip_prefix_and_eos(full_ids, prompt_ids, eos_token_id)
        special_ids = set(int(x) for x in processor.tokenizer.all_special_ids)
        text = decode_with_tokffi(tokffi_tokenizer, content_ids, special_ids)
        ...
        if tokffi_tmpdir is not None:
            tokffi_tmpdir.cleanup()
