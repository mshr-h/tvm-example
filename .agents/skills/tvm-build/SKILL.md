...
Usage
```bash
.agents/skills/tvm-build/build-tvm.sh [--clean] [--cuda] [--msc] [--papi] [--opencl] [--clml] [--llvm ON|OFF]
```
...
- Use `--cuda`, `--msc`, `--papi`, `--opencl`, and `--clml` to enable optional features supported by the script.
